...
void solve(){
	...
	map<int, int> mp;
	for (int it:a) {
		if (f(mp, it) && f(mp, it-1)) mp.insert(make_pair(it, 1));
		else if (f(mp, it) && !f(mp, it-1)) mp.insert(make_pair(it, mp[it-1]+1));
		else if (!f(mp, it) && !f(mp, it-1)) mp[it] = max(mp[it], mp[it-1] + 1);
	}
	int mx = 0, p;
	for (auto it:mp) {
		if (mx < it.ss) {
